...
int main(){
	...
	//	cout<<depan<<" "<<belakang<<endl;
		...
		if (is_rev){
			if (A[int(s[s.length()/2])-65]=='0') is_rev=false;
		}
		i=0; j=belakang.length()-1;
		...
		if (is_nat && is_rev) cout<<s<<" -- is a mirrored palindrome."<<endl<<endl;
		else if (is_nat && !is_rev) cout<<s<<" -- is a regular palindrome."<<endl<<endl;
		else if (!is_nat && is_rev) cout<<s<<" -- is a mirrored string."<<endl<<endl;
